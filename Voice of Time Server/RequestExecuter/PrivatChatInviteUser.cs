...
namespace Voice_of_Time_Server.RequestExecuter
{
    ...
    internal class PrivatChatInviteUser : IServerRequestExecuter
    {
        ...
        (IVOTPHeader, IVOTPBody?)? IServerRequestExecuter.ExecuteRequest(HeaderReq header, IVOTPBody? body, SocketHandler socket)
        {
            ...
            if (!UserChatState.HasFlag(ChatUserState.ADMIN) && !UserChatState.HasFlag(ChatUserState.MODERATOR)) // Inviter needs some privliges to invite someone. Not everyone can invite anyone
            {
            ...
            ServerData.server.AddChatUser(invite.ChatID, invite.TargetID, ChatUserState.INVITED); //TODO: IDEA: posiible instant privliges like invite as moderator or so...
